...
try:
    pgdb_conn_pool = pool.ThreadedConnectionPool(
        1, 20, host="postgres0", user="dbuser0", password="pwd0*", database="titanicdb"
    )
    ...
    def post_passenger():
        ...
        post_data = (
            ...
            _sex,
            _age,
            _siblingsOrSpousesAboard,
            _parentsOrChildrenAboard,
            _fare,
        )
        #    conn = psycopg2.connect(host='postgres0', user='dbuser0', password='pwd0*', dbname='titanicdb')
        conn = pgdb_conn_pool.getconn()
    ...
    def put_passenger(id):
        ...
        _fare = req_data["fare"]
        query = (
            'UPDATE titanic SET survived = %s, "passengerClass" = %s, name = %s, sex = %s, age = %s, "siblingsOrSpousesAboard" = %s, "parentsOrChildrenAboard" = %s, fare = %s WHERE uuid::text = '
            + "'"
            + str(id)
            + "'"
        )
